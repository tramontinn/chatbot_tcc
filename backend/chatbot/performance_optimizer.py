...
def__init__(self,max_workers:int=None):
        self.max_workers=max_workersormin(32,(mp.cpu_count()or1)+4)
...
"avg_cpu_usage":sum(m.cpu_usageforminhistory)/len(history),
...
"max_cpu_usage":max(m.cpu_usageforminhistory),
...
ifcurrent_metrics.cpu_usage>self.cpu_threshold:
            health_status["issues"].append("Alto uso de CPU")
health_status["recommendations"].append("Reduza o nmero de tarefas concorrentes")
...
"cpu_usage_percent":health_check["current_metrics"].cpu_usage,
...
ifmetrics.get("avg_cpu_usage",0)>70:
            recommendations.append("Reduza o nmero de workers ou use processamento assncrono")
